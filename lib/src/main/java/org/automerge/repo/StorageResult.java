...
/**
 * Represents the result of a storage operation task. This is an abstract base
 * class with concrete subclasses for each result type.
 */
abstract class StorageResult {
     ...
     */
    static class Load extends StorageResult {

        private final Optional<byte[]> value;

        Load(byte[] value) {
            this.value = value != null ? Optional.of(value.clone()) : Optional.empty();
        }
        ...
        public String toString() {
            return ("StorageResult.Load{found=" + isFound()
                    + (value.isPresent() ? ", valueLength=" + value.get().length : "") + "}");
        }
    }
     ...
     */
    static class LoadRange extends StorageResult {
     ...
     */
    static class Put extends StorageResult {
     ...
     */
    static class Delete extends StorageResult {
